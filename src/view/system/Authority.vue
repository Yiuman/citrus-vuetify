<template>
    <v-card class="height-100pc">
        <crud-table namespace="/rest/auth" :exclude-actions="['edit']" @action="actionEvent" ref="$crud$">
            <!--编辑页面-->
...
<script>
    ...
    export default {
        name: "Authority",
        components: {SimpleFormSlot, TreeModel, CrudTable},
        data: () => ({
            addDialog: false,
        ...
        methods: {
            ...
            saveEntity() {
                ...
                return new Promise(resolve => {
                    this.getService().save(submitEntity)
                        .then(resolve);
                })
            },
            getService() {
